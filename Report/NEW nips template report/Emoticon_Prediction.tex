...
As a message belongs to one of multiple classes, an averaged multi-class perceptron was implemented. %why average the weights?
%%% WHERE I LEFT OFF
...
Once the data is collected and preprocessed, its features are extracted. Choosing good features might significantly improve the performance in many cases, but this task is not a trivial one. Due to time constraints, certain basic features were chosen for the current research. However, it is believed that research performed in the future can explore a bigger (or different) set of features. Table ~\ref{featuretable} presents the features which were extracted and used for classification. Since most of the features' names are self-explanatory, only the some of the features are discussed in details in the following paragraphs.
